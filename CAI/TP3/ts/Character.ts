 ...
 *      }
 *      // Pas de méthodes vu le code ci-dessous
 *  }
*/
...
//      Size of images must be changed so that feature detection algorithms work better...
    ...
    this._mouth.then(function (mouth) {
        // Bouche la plus basse sur la photo mais ne tient pas compte que la 
        // bouche est incluse dans la face (voir Macron !) :
    ...
    Promise.all([this._eyes, this._face, this._mouth]).then(function (features) {
        // Une seule face calculée sinon tentative de rectification peu sensée...
        if (features[1] !== null && features[1].length === 1) { 
            /** EYES */
                    ...
                    features[0].forEach(eye => {
                        if (estimated_eye_left_x < eye.x * 0.75 || estimated_eye_left_x > eye.x / 0.75) {
                            if (estimated_eye_right_x < eye.x * 0.75 || estimated_eye_right_x > eye.x / 0.75) {
                                window.alert(name + " estimated_eye_x: " + estimated_eye_right_x + " eye.x min: " + (eye.x * 0.75) +
                        ...
                        if (estimated_eye_y < eye.y * 0.75 || estimated_eye_y > eye.y / 0.75) {
                            window.alert(name + " estimated_eye_y: " + estimated_eye_y + " eye.y min: " + (eye.y * 0.75) +
